...
AABCharacterBase::AABCharacterBase()
{
	...
	// 콤보 액션 몽타주 에셋 설정.
	static ConstructorHelpers::FObjectFinder<UAnimMontage> ComboActionMantageRef(TEXT("/Game/ArenaBattle/Animation/AM_ComboAttack.AM_ComboAttack"));
	if (ComboActionMantageRef.Object)
	{
		ComboActionMontage = ComboActionMantageRef.Object;
	}
	...
	// 죽음 몽타주 에셋 설정.
	static ConstructorHelpers::FObjectFinder<UAnimMontage> DeadMantageRef(TEXT("/Game/ArenaBattle/Animation/AM_Dead.AM_Dead"));
...
void AABCharacterBase::SetupCharacterWidget(class UUserWidget* IsUserWidget)
{
	// 필요한 위젯 정보 가져오기.
	UABHpBarWidget* HpBarWidget = Cast<UABHpBarWidget>(IsUserWidget);
	if (HpBarWidget)
	{
		// 최대 체력 값 설정.
		//HpBarWidget->SetMaxHp(Stat->GetMaxHap());
...
void AABCharacterBase::AttackHitCheck()
{
	...
	// 공격 거리.
	//constexpr float AttackRange = 50.0f;
...
void AABCharacterBase::ComboActionEnd(class UAnimMontage* TargetMontage, bool IsProperyEnded)
{
	// 유효성 검사.
...
void AABCharacterBase::DrinkPotion(class UABItemData* InItemData)
{
	...
	UABPotionItemData* PointItemData = Cast<UABPotionItemData>(InItemData);
	if (PointItemData)
	{
		// 스탯 컴포넌트에 체력 회복 처리.
		Stat->HealHp(PointItemData->HealAmount);
	}
}
